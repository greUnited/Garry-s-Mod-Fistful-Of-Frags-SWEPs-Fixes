...
SWEP.Runspeed = 500 // Default Run Speed
SWEP.Walkspeed = 250 // Default Walk Speed
SWEP.Speed = 0.5 // Slow down multiplier for ironsights
SWEP.Mass = 0.9  // Slow down multiplier for weight of swep

SWEP.ManualHolster = true // allows you to enable or disable holster.
SWEP.WeightMod = true // allows you to enable or disable speed mods.
...
function SWEP:SetIronsights(b)
   ...
   if (self.Owner) then
      if (b) then
         ...
         if (self.WeightMod) and not self.Weapon:GetDTBool(0) then
            self.Owner:SetRunSpeed(self.Runspeed*self.Mass)
            self.Owner:SetWalkSpeed(self.Walkspeed*self.Mass)
...
---------------------------------------------------------*/
-- EDIT: Can manually holster? If so then return true.
function SWEP:Holster()
   return self.ManualHolster
end

-- NOTE: Code is the old, broken one.
--       GetDTBool(0) should not be used
--       And is the issue of the below code.
